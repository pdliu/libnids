...
#error DLT_PRISM_HEADER is defined, but DLT_IEEE802_11 is not ???
        ...
        case DLT_IEEE802_11:
            ...
            */
            ...
            if (ETHERTYPE_IP != EXTRACT_16BITS(data + nids_linkoffset + LLC_OFFSET_TO_TYPE_FIELD)) 
            {
...
static void process_udp(char *data)
{
    ...
    if (udph->uh_sum 
        && my_udp_check((void *) udph, ulen, iph->ip_src.s_addr, iph->ip_dst.s_addr)) 
    {
        return;
    }
...
int nids_dispatch(int cnt)
{
    ...
    START_CAP_QUEUE_PROCESS_THREAD(); /* threading... */
    if ((r = pcap_dispatch(desc, cnt, (pcap_handler) nids_pcap_handler, NULL)) == -1) 
    {
